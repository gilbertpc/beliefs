...
- `JBM` is not very credulous in regards to `J` ($\alpha_{5,3}=0.2$) but gives no credence to `B` ($\alpha_{5,4}=0$). He starts with opinion $x_5(0)=15\%$. 
...
```{r graph_onestubborn, fig.width=13}
T <- 200
...
```{r graph_twostubborns, fig.width=13}
inits["JBM"] <- 0
alpha["JBM","J"] <- 0
T <- 1000
X <- matrix(NA, nrow=nrow(alpha), ncol=T)
...
I would finish by this metaphor by Henri Laborit: *To go to the moon we need to know the laws of gravitation. When one know these laws, this does not mean that one is freed from the laws of gravity, but it means they can be used for doing something else.*
